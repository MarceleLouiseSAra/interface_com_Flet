...
def view_add_expense_page(page: Page) -> Container:
    ...
    radio = RadioGroup(
        content=Column([Radio(value=opcao, label=opcao) for opcao in participants])
    )
    ...
    listaDeParticipantes.controls.append(
        radio
    )  # retorna o resultado de radio e retirar da listaDeDevedores quem pagou pela despesa
        ...
        content=Column(
            controls=[
                ...
                Container(
                    ...
                    content=Column(controls=[title_input]),
                ),
                Container(
                    margin=margin.only(left=15, right=10, top=15),
                    content=Column(controls=[amount_input]),
                ),
                Row(
                    [
                        Container(
